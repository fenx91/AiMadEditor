...
## 6. 项目目录结构 (Project Directory Structure)
...
│   ├── app.py                # 后端主入口，提供 HTTP/WebSocket 接口服务
│   ├── analyzer.py           # 音频 BPM 节奏分析与歌词高精度物理裁剪
│   ├── indexer.py            # 视频关键帧提取、ASR 语音转写、CLIP/Whisper 向量索引生成
│   ├── matcher.py            # 双轨语义打分融合与画面检索匹配
│   ├── qrc_to_lrc.py         # 音频歌词格式转换工具
│   └── requirements.txt      # 后端依赖配置
│
├── frontend/                 # 前端工作区目录 (Web SPA)
│   ├── index.html            # Web 端渲染主页
│   ├── app.js                # 前端核心业务逻辑，含 Ping-Pong 双播放器预加载及 Timeline 状态机
│   └── style.css             # 前端样式定义（暗色调 UI 与时间轴布局）
│
├── hyperframes_template/     # Headless 浏览器渲染所需的 HTML 模板与渲染元数据
│
├── data/                     # 核心资产存储与脚本库
│   ├── metadata.db           # SQLite 数据库，保存视频、关键帧及 ASR 语义向量
│   ├── keyframes/            # 提取的每 0.5s 关键帧图像
│   ├── proxies/              # 自动转码生成的 H.264 视频代理（供前端播放器极速预览）
│   ├── helper/               # 辅助维护脚本（DB 校验、后填补、角色人像提取等）
│   └── ...                   # 音频缓存、人像及音乐临时文件夹
│
├── in/                       # 视频素材与字幕导入根目录
│   └── subtitles/            # ASS/SRT 字幕文件夹
│
├── tests/                    # 自动化测试套件
│   ├── test_analyzer.py      # 音频节奏分析单元测试
│   ├── test_trimmer.py       # 歌词偏置与裁剪单元测试
│   ├── test_matcher.py       # 检索匹配单元测试
│   ├── test_voice_index.py   # 语音识别与打分单元测试
│   └── ...                   # CLIP/Gemini 真实 API 集成测试
│
├── node/                     # 预构建的本地 Node.js 运行环境 (Headless 渲染)
├── run_tests.sh              # 单元测试启动脚本
└── run_real_render.sh        # 渲染测试启动脚本
```
